...
// Box is either null or an object with top/left/width/height in pixels
...
export default function HoverReceiver() {
  ...
  useEffect(() => {
    function onPointerMove(e) {
      ...
      const hit = document.elementFromPoint(e.clientX, e.clientY)?.closest && document.elementFromPoint(e.clientX, e.clientY).closest("[data-orchids-id]") || null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        if (!hit) {
          ...
          flushImageSrcChange();
          const msg = { type: CHANNEL, msg: "HIT", id: null, tag: null, rect: null };
          postMessageDedup(msg);
          return;
        }
        ...
        const msg = { type: CHANNEL, msg: "HIT", id: hitId, tag: tagName, rect: hitId !== focusedElementId ? expandBox(hit.getBoundingClientRect()) : null };
        postMessageDedup(msg);
      }
    }
    ...
    function onMouseDownCapture(e) {
      ...
      const hit = (e.target && e.target.closest) ? e.target.closest("[data-orchids-id]") : null;
    ...
    function onClickCapture(e) {
      ...
      const target = e.target;
      const hit = target.closest && target.closest("[data-orchids-id]") ? target.closest("[data-orchids-id]") : null;
      ...
      if (hit) {
        ...
        const isLink = hit.tagName.toLowerCase() === "a" || !!hit.closest && !!hit.closest("a");
        ...
        if (hitId && typeof window !== "undefined") {
          const focusedElementData = { id: hitId, tag: tagName };
        ...
        if (isEditable) {
          ...
          if (hit !== editingElementRef.current) {
            ...
            if (hit.childElementCount > 0) originalContentRef.current = extractDirectTextContent(hit);
            else originalContentRef.current = hit.innerText || hit.textContent || "";

            const createHandlers = (element) => {
              const handleFocus = () => {
                if (element !== editingElementRef.current) return;
                handleStyleBlur(element);
                if (element.childElementCount > 0) originalContentRef.current = extractDirectTextContent(element);
                else originalContentRef.current = element.innerText || element.textContent || "";
                hasStyleChangesRef.current = false;
              };
        ...
        const msg = {
          ...
          id: hitId,
          tag: tagName,
          rect: expandedBox ? { top: expandedBox.top, left: expandedBox.left, width: expandedBox.width, height: expandedBox.height } : { top: 0, left: 0, width: 0, height: 0 },
        ...
        if (focusedElementRef.current) {
          ...
          const msg = { type: CHANNEL, msg: "ELEMENT_CLICKED", id: null, tag: null, rect: { top: 0, left: 0, width: 0, height: 0 }, clickPosition: { x: e.clientX, y: e.clientY }, isEditable: false, currentStyles: {}, className: "" };
          postMessageDedup(msg);
        }
      }
    }
  ...
  return (
    <>
      ...
      {isVisualEditMode && focusBox && (
        <>
          {focusTag && (
            <div className="fixed text-[10px] font-semibold text-white bg-[#3b82f6] px-1 rounded-sm pointer-events-none select-none" style={{ zIndex: 100003, left: focusBox.left - 4, top: focusBox.top - 16 }}>
              {focusTag}
            </div>
          )}

          <div className="fixed pointer-events-none border-[1.5px] border-[#38bdf8] rounded-sm" style={{ zIndex: 100001, left: focusBox.left, top: focusBox.top, width: focusBox.width, height: focusBox.height }} />

          {!isResizing && (
            <>
              <div className="fixed w-2 h-2 bg-[#38bdf8] rounded-full cursor-nw-resize pointer-events-auto resize-handle" style={{ zIndex: 100002, left: focusBox.left - 4, top: focusBox.top - 4 }} onMouseDown={(e) => handleResizeStart(e, "nw")} />
              <div className="fixed w-2 h-2 bg-[#38bdf8] rounded-full cursor-ne-resize pointer-events-auto resize-handle" style={{ zIndex: 100002, left: focusBox.left + focusBox.width - 4, top: focusBox.top - 4 }} onMouseDown={(e) => handleResizeStart(e, "ne")} />
              <div className="fixed w-2 h-2 bg-[#38bdf8] rounded-full cursor-sw-resize pointer-events-auto resize-handle" style={{ zIndex: 100002, left: focusBox.left - 4, top: focusBox.top + focusBox.height - 4 }} onMouseDown={(e) => handleResizeStart(e, "sw")} />
              <div className="fixed w-2 h-2 bg-[#38bdf8] rounded-full cursor-se-resize pointer-events-auto resize-handle" style={{ zIndex: 100002, left: focusBox.left + focusBox.width - 4, top: focusBox.top + focusBox.height - 4 }} onMouseDown={(e) => handleResizeStart(e, "se")} />

              <div className="fixed w-2 h-2 bg-[#38bdf8] rounded-full cursor-n-resize pointer-events-auto resize-handle" style={{ zIndex: 100002, left: focusBox.left + focusBox.width / 2 - 4, top: focusBox.top - 4 }} onMouseDown={(e) => handleResizeStart(e, "n")} />
              <div className="fixed w-2 h-2 bg-[#38bdf8] rounded-full cursor-s-resize pointer-events-auto resize-handle" style={{ zIndex: 100002, left: focusBox.left + focusBox.width / 2 - 4, top: focusBox.top + focusBox.height - 4 }} onMouseDown={(e) => handleResizeStart(e, "s")} />
              <div className="fixed w-2 h-2 bg-[#38bdf8] rounded-full cursor-w-resize pointer-events-auto resize-handle" style={{ zIndex: 100002, left: focusBox.left - 4, top: focusBox.top + focusBox.height / 2 - 4 }} onMouseDown={(e) => handleResizeStart(e, "w")} />
              <div className="fixed w-2 h-2 bg-[#38bdf8] rounded-full cursor-e-resize pointer-events-auto resize-handle" style={{ zIndex: 100002, left: focusBox.left + focusBox.width - 4, top: focusBox.top + focusBox.height / 2 - 4 }} onMouseDown={(e) => handleResizeStart(e, "e")} />
            </>
          )}
        </>
      )}
